...
async function translateContent(content, targetLang) {
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `You are a professional translator. Translate the following markdown content to ${targetLang}. Keep all markdown formatting, code blocks, and links intact. Only translate visible text. Do not translate metadata ids. Prefer industry-standard and commonly used technical terms over literal translations, and ensure terminology consistency throughout the document.`
      },
      {
        role: 'user',
        content: content
